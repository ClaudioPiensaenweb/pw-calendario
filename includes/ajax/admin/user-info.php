...
echo '<div class="booked-scrollable">';
	...
	echo '<p class="booked-title-bar"><small>'.esc_html__('Editar la cita','pw-calendario').'</small></p>';
	...
	?><form action="" method="post" class="booked-form" id="editAppointmentForm" data-appt-id="<?php echo $appt_id; ?>"<?php if ($calendar_id): echo ' data-calendar-id="'.$calendar_id.'"'; endif; ?>><?php
		...
		$user_id = ( isset($_POST['user_id']) ? pwcal_post_entero( 'user_id' ) : false );

		if ( $appt_id ):

			?><p class="booked-calendar-name"><?php esc_html_e( 'Fecha y hora de la cita', 'pw-calendario' ); ?></p><?php
			...
			?><p class="booked-calendar-name"><?php esc_html_e( 'Datos del cliente', 'pw-calendario' ); ?></p><?php
			...
			if (isset($name_requirements[0]) && $name_requirements[0] == 'require_surname'): ?>
				<div class="field">
					<input value="<?php echo esc_attr( $first_name ); ?>" placeholder="<?php esc_html_e('Nombre','pw-calendario'); ?>..." type="text" class="textfield" name="name"<?php if ( $user_id && user_can( $user_id, 'manage_options' ) ):
						echo ' readonly="true"';
					endif; ?> />
					<input value="<?php echo esc_attr( $last_name ); ?>" placeholder="<?php esc_html_e('Apellidos','pw-calendario'); ?>..." type="text" class="textfield" name="surname"<?php if ( $user_id && user_can( $user_id, 'manage_options' ) ):
						echo ' readonly="true"';
					endif; ?> />
				</div>
			<?php else: ?>
				<div class="field">
					<input
						value="<?php echo esc_attr( $first_name . ( $last_name ? ' ' . $last_name : '' ) ); ?>"
						placeholder="<?php esc_html_e('Nombre','pw-calendario'); ?>..."
						type="text"
			...
			if ($customer_email):
				?><div class="field">
					<input
						...
						placeholder="<?php esc_html_e('Correo electrónico','pw-calendario'); ?>..."
						type="email"
						class="large textfield"
						name="email"<?php
						if ( $user_id && user_can( $user_id, 'manage_options' ) ):
			...
			if ($customer_phone):
				?><div class="field">
					<input value="<?php echo esc_attr( $customer_phone ); ?>" placeholder="<?php esc_html_e('Teléfono','pw-calendario'); ?>..." type="tel" class="large textfield" name="phone"<?php
						if ( $user_id && user_can( $user_id, 'manage_options' ) ):
							echo ' readonly="true"';
						endif;
					?> />
			...
			if ( $cf_meta_value ):
				...
				?><p class="booked-calendar-name"><?php esc_html_e( 'Datos de la cita', 'pw-calendario' ); ?></p><?php
			...
			?><div class="field">
				<input type="submit" class="button button-primary" value="<?php esc_html_e('Actualizar la cita','pw-calendario'); ?>">
			</div><?php
