...
if(!class_exists('Booked_Admin_AJAX')) {
	class Booked_Admin_AJAX {
		...
		public function booked_admin_disable_slot(){
			...
			pwcal_verificar_ajax_admin();

			$date = pwcal_post_fecha( 'date' );
			$date = date( 'Y-m-d', strtotime( $date ) );
			$timeslot = pwcal_post_intervalo( 'timeslot' );
			$calendar_id = pwcal_post_calendario( 'calendar_id' );
		...
		public function booked_date_formatting(){
			...
			if (isset($_POST['date']) && $_POST['date']):
				...
				echo ucwords( date_i18n( $date_format,strtotime(pwcal_post_fecha( 'date' )) ) );
			else:
				echo '';
			endif;
		...
		public function booked_admin_add_appt(){
			...
			if (isset($_POST['date']) && isset($_POST['timestamp']) && isset($_POST['timeslot']) && isset($_POST['customer_type'])):
				...
				include(PWCAL_AJAX_INCLUDES_DIR . 'admin/add-appointment.php');
			...
			if ( is_user_logged_in() && current_user_can( 'edit_booked_appointments' ) ):
				...
				if ( isset($_POST['appt_date']) && isset($_POST['appt_timeslot']) && isset($_POST['name']) ):
					...
					if ( !$email_required || $email_required && isset($_POST['email']) ):
						...
						include(PWCAL_AJAX_INCLUDES_DIR . 'admin/edit-appointment.php');
				...
				echo 'error###' . esc_html__( "Tu perfil de usuario no tiene permisos para editar citas.", 'pw-calendario' );
		...
		public function booked_admin_delete_custom_timeslot(){
			...
			if (isset($_POST['timeslot']) && isset($_POST['currentArray'])):
				...
				$timeslot_to_delete = pwcal_post_intervalo( 'timeslot' );
				$current_times = pwcal_post_json( 'currentArray' );
				$current_times_details = pwcal_post_json( 'currentArrayDetails' );

				do_action('booked_deleting_custom_timeslot',pwcal_post_fecha( 'start_date' ),pwcal_post_fecha( 'end_date' ),pwcal_post_intervalo( 'timeslot' ),pwcal_post_calendario( 'calendar_id' ));
		...
		public function booked_admin_adjust_custom_timeslot_count(){
			...
			if (isset($_POST['currentArray']) && isset($_POST['timeslot']) && isset($_POST['newCount'])):
				...
				$current_times = pwcal_post_json( 'currentArray' );
				$timeslot = pwcal_post_intervalo( 'timeslot' );
				$newCount = pwcal_post_entero( 'newCount' );
		...
		public function booked_admin_add_custom_timeslot(){
			...
			if (isset($_POST['startTime']) && isset($_POST['endTime']) && isset($_POST['count']) && isset($_POST['currentTimes'])):
				...
				$startTime = pwcal_post_texto( 'startTime' );
				$endTime = pwcal_post_texto( 'endTime' );
				$count = pwcal_post_entero( 'count' );
				$current_times = pwcal_post_json( 'currentTimes' );
				$current_times_details = !empty($_POST['currentTimesDetails']) ? pwcal_post_json( 'currentTimesDetails' ) : array();
				...
				do_action('booked_creating_custom_timeslot',pwcal_post_fecha( 'start_date' ),pwcal_post_fecha( 'end_date' ),$startTime,$endTime,pwcal_post_calendario( 'calendar_id' ),$title);
		...
		public function booked_admin_add_custom_timeslots(){
			...
			if (isset($_POST['startTime']) && isset($_POST['endTime']) && isset($_POST['interval']) && isset($_POST['count']) && isset($_POST['time_between'])):
				...
				$startTime = pwcal_post_texto( 'startTime' );
				$endTime = pwcal_post_texto( 'endTime' );
				if (pwcal_post_texto( 'endTime' ) == '2400'):
					$endTime = '2400';
				endif;

				$interval = pwcal_post_texto( 'interval' );
				$count = pwcal_post_entero( 'count' );
				$time_between = pwcal_post_texto( 'time_between' );
				$current_times = pwcal_post_json( 'currentTimes' );
				$current_times_details = !empty($_POST['currentTimesDetails']) ? pwcal_post_json( 'currentTimesDetails' ) : array();
				...
				do {
					...
					do_action('booked_creating_custom_timeslot',pwcal_post_fecha( 'start_date' ),pwcal_post_fecha( 'end_date' ),$startTime,$newStartTime,pwcal_post_calendario( 'calendar_id' ),$title);
		...
		public function booked_admin_save_custom_fields(){
			...
			if (isset($_POST['booked_custom_fields'])):
				...
				$custom_fields = pwcal_post_texto( 'booked_custom_fields' );
				$calendar_id = pwcal_post_entero( 'booked_cf_calendar_id' );
				if ($custom_fields != '[]'):
					if ($calendar_id):
		...
		public function booked_admin_delete_appt(){
			...
			if (isset($_POST['appt_id'])):
				...
				$appt_id = pwcal_post_entero( 'appt_id' );
				include(PWCAL_AJAX_INCLUDES_DIR . 'admin/delete-appointment.php');
		...
		public function booked_admin_get_timeslots_select(){
			...
			$appt_id = ( isset($_POST['appt_id'] ) ? pwcal_post_entero( 'appt_id' ) : false );
		...
		public function booked_admin_custom_timeslots_list(){
			...
			if (isset($_POST['json_array'])):
				...
				$this_timeslot = array();
				$this_timeslot['booked_custom_start_date'] = pwcal_post_fecha( 'start_date' );
				if (pwcal_post_fecha( 'end_date' )): $this_timeslot['booked_custom_end_date'] = pwcal_post_fecha( 'end_date' ); else: $this_timeslot['booked_custom_end_date'] = pwcal_post_fecha( 'start_date' ); endif;
				$calendar_id = pwcal_post_calendario( 'calendar_id' );

				$timeslots = pwcal_post_json( 'json_array' );
				$timeslots_detailed = !empty($_POST['json_array_detailed']) ? pwcal_post_json( 'json_array_detailed' ) : array();
		...
		public function booked_admin_calendar_picker(){
			...
			if (isset($_POST['gotoMonth'])):
				...
				$timestamp = (pwcal_post_texto( 'gotoMonth' ) != 'false' ? strtotime(pwcal_post_texto( 'gotoMonth' )) : current_time('timestamp'));
		...
		public function booked_admin_user_info_modal(){
			...
			if (isset($_POST['user_id'])):
				...
				include(PWCAL_AJAX_INCLUDES_DIR . 'admin/user-info.php');
		...
		public function booked_admin_new_appointment_form(){
			...
			if (isset($_POST['date']) && isset($_POST['timeslot'])):
				...
				include(PWCAL_AJAX_INCLUDES_DIR . 'admin/appointment-form.php');
