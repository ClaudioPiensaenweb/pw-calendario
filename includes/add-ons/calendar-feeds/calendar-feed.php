...
// El secreto se compara con `hash_equals` para no filtrar informacion por
// el tiempo de respuesta, y con comparacion estricta de tipos: el `!=`
// original permitia saltarse la comprobacion con malabares de tipos.
$pwcal_secreto_recibido = isset( $_GET['sh'] ) ? (string) wp_unslash( $_GET['sh'] ) : '';

if ( ! $pwcal_secreto_recibido || ! hash_equals( (string) BOOKEDICAL_SECURE_HASH, $pwcal_secreto_recibido ) ) {
	status_header( 403 );
	wp_die(
		esc_html__( 'Este feed de calendario requiere una clave de acceso válida. Consulta las URL actualizadas en Citas > Ajustes > Feeds de calendario.', 'pw-calendario' ),
		esc_html__( 'Acceso no autorizado', 'pw-calendario' ),
		array( 'response' => 403 )
	);
}

header('Content-type: text/calendar; charset=utf-8');
header('Content-Disposition: attachment; filename=appointment-feed-'.PWCAL_VERSION.'.ics');
...
PRODID:-//piensaenweb.com//Pw Calendario<?php echo "\r\n"; ?>
		...
		if (!$guest_name):
			...
			if ($user_id):
				...
				if ( !empty($user_info) ):
					...
					$display_name = esc_html__('[Sin nombre]','pw-calendario');
					$email = esc_html__('[Sin correo]','pw-calendario');
				endif;
			else:
				$display_name = esc_html__('[Sin nombre]','pw-calendario');
				$email = esc_html__('[Sin correo]','pw-calendario');
...
?>SUMMARY:<?php echo pwcal_escapar_ics( apply_filters( 'booked_calendar_feed_display_name', $display_name, $appt_id ) ); ?><?php echo "\r\n"; ?>
...
// `pwcal_escapar_ics()` vive ahora en includes/seguridad.php,
// porque tambien la usa el boton "Anadir al calendario".
