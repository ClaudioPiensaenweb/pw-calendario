...
class Booked_WC_Appointment {
	...
	public static function get( $post_id=null ) {
		if ( !is_integer($post_id) ) {
			$message = sprintf( __('Se esperaba un entero (%s) y se ha recibido %s.', 'pw-calendario'), 'Booked_WC_Appointment::get($post_id)', gettype($post_id) );
		...
		if ( !empty($this->timeslot) ):
			...
			if ( $show_only_titles && $this->title ){
				...
				if ($timeslots[0] == '0000' && $timeslots[1] == '2400') {
					$timeslotText = $day_year . ' (' . esc_html__('Todo el día','pw-calendario') . ')';
				} else if ( !$hide_end_times ) {
					$timeslotText = sprintf(__('de %1$s a %2$s del %3$s', 'pw-calendario'), $time_start, $time_end, $day_year);
				} else if ( $hide_end_times ) {
					$timeslotText = sprintf(__('a las %1$s del %2$s', 'pw-calendario'), $time_start, $day_year);
				} else {
					$timeslotText = 'N/A';
				}
