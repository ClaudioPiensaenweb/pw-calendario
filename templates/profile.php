...
if (empty($user_data)) {
	...
	echo '<h2>' . esc_html__('Aquí no hay ningún perfil.','pw-calendario') . '</h2>';
	echo '<p>' . esc_html__('Este perfil de usuario no existe.','pw-calendario') . '</p>';
		...
		<div class="booked-info">
			<div class="booked-user">
				...
				<h3 class="<?php echo $h3_class; ?>">
					<?php echo sprintf(esc_html__('Hola de nuevo, %s.','pw-calendario'),'<strong>'.booked_get_name( $user_data->ID ).'</strong>'); ?>
					<?php if ($my_profile): ?>
						&nbsp;&nbsp;<a class="booked-logout-button" href="<?php echo wp_logout_url(get_permalink($post->ID)); ?>" title="<?php esc_html_e('Salir','pw-calendario'); ?>"><?php esc_html_e('Salir','pw-calendario'); ?></a>
					<?php endif; ?>
				</h3>
			</div>
		</div>
	...
	<ul class="booked-tabs bookedClearFix">
		<?php
			...
			$default_tabs = array(
				'appointments' => array(
					'title' => esc_html__('Citas próximas','pw-calendario'),
					'booked-icon' => 'booked-calendar',
					'class' => false
				),
				'history' => array(
					'title' => esc_html__('Historial de citas','pw-calendario'),
					'booked-icon' => 'booked-calendar-check-o',
					'class' => false
				),
				'edit' => array(
					'title' => esc_html__('Editar el perfil','pw-calendario'),
