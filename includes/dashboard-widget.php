...
class BookedDashboardWidget {
	...
	public function booked_dashboard_widget() {
		...
		wp_add_dashboard_widget(
	        ...
	        '<i class="booked-icon booked-icon-calendar"></i>&nbsp;&nbsp;'.esc_html__('Citas próximas','pw-calendario'),
		...
		echo '<div class="booked-pending-appt-list booked-dashboard-widget">';
			...
			*/
			...
			foreach($appointments_array as $appt):
				...
				echo '<div class="pending-appt bookedClearFix" data-appt-id="'.esc_attr($appt['post_id']).'">';
					...
					echo '<span class="appt-block" data-appt-id="'.esc_attr($appt['post_id']).'">';
						...
						if (!isset($appt['guest_name'])):
							...
							if (isset($user_info->ID)):
								echo '<a href="#" class="user" data-user-id="'.esc_attr($user_info->ID).'"><i class="booked-icon booked-icon-pencil"></i>&nbsp;'.esc_html(booked_get_name($user_info->ID)).'</a>';
							else :
								esc_html_e('(este usuario ya no existe)','pw-calendario');
							endif;
						else :
							echo '<a href="#" class="user" data-user-id="0"><i class="booked-icon booked-icon-pencil"></i>&nbsp;'.esc_html($appt['guest_name']).'</a>';
						endif;
			...
			echo '<div class="pending-appt'.(!empty($appointments_array) ? ' no-pending-message' : '').'">';
				echo '<p style="text-align:center;">'.esc_html__('No hay citas próximas.','pw-calendario').'</p>';
			echo '</div>';
