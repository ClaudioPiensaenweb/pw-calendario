...
class BookedShortcodes {
	...
	public function booked_registration_redirect(){
		...
		if ( get_option('users_can_register') && !is_user_logged_in() && isset($_POST['booked_reg_submit'] ) ) {
			...
			if ( $name_requirements == 'require_surname' && isset($_POST['booked_reg_surname']) && !$_POST['booked_reg_surname'] ):
				...
				$booked_reg_errors[] = esc_html__('Para registrarte tienes que indicar el nombre y los apellidos.','pw-calendario');

			elseif ( !isset($_POST['booked_reg_name']) || isset($_POST['booked_reg_name']) && !$_POST['booked_reg_name'] ):

				$registration_complete = 'error';
				$booked_reg_errors[] = esc_html__('Para registrarte tienes que indicar tu nombre.','pw-calendario');

			else:

				$name 			= isset($_POST['booked_reg_name']) ? pwcal_post_texto( 'booked_reg_name' ) : false;
				$surname 		= isset($_POST['booked_reg_surname']) ? pwcal_post_texto( 'booked_reg_surname' ) : false;
		        ...
		        $email      	= isset($_POST['booked_reg_email']) ? sanitize_email(pwcal_post_texto( 'booked_reg_email' )) : '';
	...
	public function booked_profile_template(){
		if (!is_user_logged_in()) {
			...
			ob_start();
			require(PWCAL_PLANTILLAS_DIR . 'profile.php');
			return ob_get_clean();
		}
	}
		...
		if ( is_user_logged_in() ):
			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="booked-profile-page" class="booked-shortcode">'; endif;
				...
				echo '<div class="booked-profile-appt-list">';
					...
					if ($historic):
						if ($total_appts):
							echo '<h4><span class="count">' . number_format($total_appts) . '</span> ' . _n('Cita pasada','Citas pasadas',$total_appts,'pw-calendario') . '</h4>';
						else:
							echo '<p class="booked-no-margin">'.esc_html__('No hay citas pasadas.','pw-calendario').'</p>';
						endif;
					else:
						if ($total_appts):
							echo '<h4><span class="count">' . number_format($total_appts) . '</span> ' . _n('Cita próxima','Citas próximas',$total_appts,'pw-calendario') . '</h4>';
						else:
							echo '<p class="booked-no-margin">'.esc_html__('No hay citas próximas.','pw-calendario').'</p>';
						endif;
					endif;
					...
					foreach($appointments_array as $appt):
						...
						if ($date_display == $today){
							$date_display = esc_html__('Hoy','pw-calendario');
							$day_name = '';
						} else {
						...
						$timeslotText = '';
						$status = ($appt['status'] != 'publish' && $appt['status'] != 'future' ? esc_html__('pendiente','pw-calendario') : esc_html__('aprobada','pw-calendario'));
						...
						if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
							...
							if ($only_titles && !$ts_title || !$only_titles):
								$timeslotText = (!get_option('booked_hide_end_times') ? esc_html__('de','pw-calendario').' ' : esc_html__('a las','pw-calendario').' ') . $time_start . (!get_option('booked_hide_end_times') ? ' &ndash; '.$time_end : '');
							endif;
							$atc_date_startend_end = $atc_date_startend;
						endif;

						echo '<span class="appt-block bookedClearFix '.(!$historic ? $status_class : 'approved').'" data-appt-id="'.esc_attr($appt['post_id']).'">';
							if (!$historic):
							...
							echo (!empty($appt['calendar_id']) ? '<i class="booked-icon booked-icon-calendar"></i><strong>'.esc_html__('Calendario','pw-calendario').':</strong> '.$appt['calendar_id'][0]->name.'<br>' : '');
							...
							echo ($cf_meta_value ? '<br><i class="booked-icon booked-icon-info"></i><a href="#" class="booked-show-cf">'.esc_html__('Información adicional','pw-calendario').'</a><div class="cf-meta-values-hidden">'.$cf_meta_value.'</div>' : '');
							...
							if (!$historic):
								...
								if ( apply_filters('booked_shortcode_appointments_allow_cancel', true, $appt['post_id']) && !get_option('booked_dont_allow_user_cancellations',false) ) { if ( $appt_date_time >= $date_to_compare ) { echo '<a href="#" data-appt-id="'.esc_attr($appt['post_id']).'" class="cancel">'.esc_html__('Cancelar la cita','pw-calendario').'</a>'; } }
			...
			return '<p>'.esc_html__('Accede para ver tus citas próximas.','pw-calendario').'</p>';
			...
			?><div id="booked-profile-page">
				...
				<div id="booked-page-form">
					...
					<ul class="booked-tabs login bookedClearFix">
						<li<?php if ( !isset($_POST['booked_reg_submit'] ) ) { ?> class="active"<?php } ?>><a href="#login"><i class="booked-icon booked-icon-lock"></i><?php esc_html_e('Acceder','pw-calendario'); ?></a></li>
						<?php if ( get_option('users_can_register') ): ?><li<?php if ( isset($_POST['booked_reg_submit'] ) ) { ?> class="active"<?php } ?>><a href="#register"><i class="booked-icon booked-icon-pencil"></i><?php esc_html_e('Registrarse','pw-calendario'); ?></a></li><?php endif; ?>
						<li><a href="#forgot"><i class="booked-icon booked-icon-question-circle"></i><?php esc_html_e('He olvidado la contraseña','pw-calendario'); ?></a></li>
					</ul>
					...
					<div id="profile-login" class="booked-tab-content">
						...
						<?php if (isset($reset) && $reset == true) { ?>
							...
							<strong><?php esc_html_e('¡Hecho!','pw-calendario'); ?></strong><br />
							<?php esc_html_e('Revisa tu correo electrónico para restablecer la contraseña.','pw-calendario'); ?>
							</p>
						...
						<div class="booked-form-wrap bookedClearFix">
							<div class="booked-custom-error"><?php esc_html_e('Los dos campos son obligatorios para acceder.','pw-calendario'); ?></div>
							<?php if (isset($_GET['loginfailed'])): ?><div class="booked-custom-error not-hidden"><?php esc_html_e('El usuario o la contraseña no son correctos.','pw-calendario'); ?></div><?php endif; ?>
							...
							echo wp_login_form( array( 'echo' => false, 'redirect' => get_the_permalink($login_redirect), 'label_username' => esc_html__( 'Correo electrónico','pw-calendario' ) ) );
					...
					<div id="profile-forgot" class="booked-tab-content">
						<div class="booked-form-wrap bookedClearFix">
							<div class="booked-custom-error"><?php esc_html_e('Necesitas indicar un correo electrónico para restablecer la contraseña.','pw-calendario'); ?></div>
							<form method="post" action="<?php echo site_url('wp-login.php?action=lostpassword', 'login_post') ?>" class="wp-user-form">
								<p class="username">
									<label for="user_login"><?php esc_html_e('¿Cuál es tu correo electrónico?','pw-calendario'); ?></label>
									<input type="text" name="user_login" value="" size="20" id="user_login" tabindex="1001" />
								</p>

								<?php do_action('login_form', 'resetpass'); ?>
								<input type="submit" name="user-submit" value="<?php esc_html_e('Restablecer mi contraseña','pw-calendario'); ?>" class="user-submit button-primary" tabindex="1002" />
