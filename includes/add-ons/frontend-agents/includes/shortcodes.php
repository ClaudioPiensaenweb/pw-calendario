		...
		if (is_user_logged_in() && current_user_can('edit_booked_appointments')):
			...
			if (!isset($atts['remove_wrapper'])): echo '<div id="booked-profile-page" class="booked-shortcode">'; endif;
				...
				echo '<div class="booked-fea-appt-list">';
					...
					if ($pending):
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s cita pendiente','%s citas pendientes',$total_appts,'pw-calendario'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No hay citas pendientes.','pw-calendario').'</p>';
						endif;
					elseif ($historic):
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s cita pasada','%s citas pasadas',$total_appts,'pw-calendario'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No hay citas pasadas.','pw-calendario').'</p>';
						endif;	
					else:
						if ($total_appts):
							echo '<h4>'.sprintf(_n('%s cita próxima','%s citas próximas',$total_appts,'pw-calendario'),'<span class="count">'.number_format($total_appts).'</span>').'</h4>';
						else:
							echo '<p class="booked-no-margin">'.__('No hay citas próximas.','pw-calendario').'</p>';
						endif;
					endif;
					...
					foreach($appointments_array as $appt):
						...
						if ($date_display == $today){
							$date_display = __('Hoy','pw-calendario');
							$day_name = '';
						} else {
						...
						if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
							$timeslotText = __('Todo el día','pw-calendario');
							$google_date_startend_end = date('Ymd',strtotime(date('Y-m-d',$appt['timestamp']) . '+ 1 Day'));
							$google_time_end = '0000';
						else :
							$timeslotText = (!get_option('booked_hide_end_times') ? __('de','pw-calendario').' ' : __('a las','pw-calendario').' ') . $time_start . (!get_option('booked_hide_end_times') ? ' ' . __('a','pw-calendario').' '.$time_end : '');
							$google_date_startend_end = $google_date_startend;
						endif;

						$status = ($appt['status'] !== 'publish' && $appt['status'] !== 'future' ? __('pendiente','pw-calendario') : __('aprobada','pw-calendario'));
						$status_class = $appt['status'] !== 'publish' && $appt['status'] !== 'future' ? 'pending' : 'approved';
						
						echo '<div class="appt-block bookedClearFix" data-appt-id="'.esc_attr($appt['post_id']).'">';
							...
							$default_button_html = '<div class="booked-fea-buttons">';
								...
								$default_button_html .= ($status_class == 'pending' ? '<button data-appt-id="'.esc_attr($appt['post_id']).'" class="approve button button-primary">'.__('Aprobar','pw-calendario').'</button>' : '');
							...
							if (!isset($appt['guest_name'])):
								...
								if (isset($user_info->ID)):
									if ($user_info->user_firstname):
										$user_display = '<a href="#" class="user" data-user-id="'.esc_attr($appt['user']).'">'.esc_html($user_info->user_firstname.($user_info->user_lastname ? ' '.$user_info->user_lastname : '')).'</a>';
										$display_name = $user_info->user_firstname.($user_info->user_lastname ? ' '.$user_info->user_lastname : '');
									elseif ($user_info->display_name):
										$user_display = '<a href="#" class="user" data-user-id="'.esc_attr($appt['user']).'">'.esc_html($user_info->display_name).'</a>';
										$display_name = $user_info->display_name;
									else:
										$user_display = '<a href="#" class="user" data-user-id="'.esc_attr($appt['user']).'">'.esc_html($user_info->user_login).'</a>';
										$display_name = $user_info->user_login;
									endif;
								else :
									$user_display = __('(este usuario ya no existe)','pw-calendario');
								endif;
							else :
								$user_display = '<a href="#" class="user" data-user-id="0">'.esc_html($appt['guest_name']).'</a>';
								$display_name = $appt['guest_name'];
							endif;
						
							echo $user_display;
							echo '<br>';
							if ($late_date > $date_to_compare): echo '<span class="late-appt">' . __('Esta cita ya ha pasado.','pw-calendario') . '</span><br>'; endif;
							...
							echo ($cf_meta_value ? '<br><i class="booked-icon booked-icon-info"></i><a href="#" class="booked-show-cf">'.__('Información adicional','pw-calendario').'</a><div class="cf-meta-values-hidden">'.$cf_meta_value.'</div>' : '');
			...
			return '<p>'.__('Accede para ver tus citas próximas.','pw-calendario').'</p>';
