...
<div class="bookings">
	...
	if ( !$total_calendars ) {
		esc_html_e( 'No hay reservas disponibles', 'pw-calendario' );
		return;
	}
		...
		foreach ( $bookings as $calendar_id => $appointments ): ?>
			...
			<div class="booked-appointments">
				...
				foreach ($appointments as $appointment_key => $appointment):
					include( PWCAL_AJAX_INCLUDES_DIR . 'front/appointment-form/appointment.php' );
					$appointment_counter++;
				endforeach;
				?>
			</div><?php
			...
			if ( get_option('users_can_register') && !$is_user_logged_in && $customer_type != 'guest' ) {
				include( PWCAL_AJAX_INCLUDES_DIR . 'front/appointment-form/form-fields-registration.php' );
			} elseif ( !$is_user_logged_in && $customer_type == 'guest' ) {
				include( PWCAL_AJAX_INCLUDES_DIR . 'front/appointment-form/form-fields-guest.php' );
			}
		...
		<div class="booked-tabs">
			<div class="booked-tabs-nav">
				<?php $i = 0; foreach ( $bookings as $calendar_id => $appointments ): ?>
					<?php
					$calendar_title = esc_html__('Desconocido', 'pw-calendario');

					if ( ! $calendar_id ) {
						$calendar_title = esc_html__('Predeterminado', 'pw-calendario');
					}
