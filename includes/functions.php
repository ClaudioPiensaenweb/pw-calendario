				...
				<th colspan="<?php if (!$hide_weekends): ?>7<?php else: ?>5<?php endif; ?>">
					...
					<span class="monthName">
						...
						<?php if ($monthShown != $currentMonth): ?>
							<a href="#" class="backToMonth" data-goto="<?php echo $currentMonth; ?>"><?php esc_html_e('Volver a','pw-calendario'); ?> <?php echo date_i18n('F',strtotime($currentMonth)); ?></a>
						<?php endif; ?>
					</span>
			...
			for($i = 1; $i <= $rows; $i++){
				...
				for($x = 1; $x <= 7; $x++){
					...
					if ($hide_weekends && $day_of_week == 6 || $hide_weekends && $day_of_week == 7):
						...
						$html .= '<span class="date'.(!$hide_available_count && $appointments_left > 0 && !in_array('prev-date',$classes) && !in_array('blur',$classes) ? ' tooltipster" title="'.sprintf( _n('%d disponibles','%d disponibles',$appointments_left,'pw-calendario'),$appointments_left) : (!$hide_available_count && !$appointments_left && $booked_pa_active && !in_array('prev-date',$classes) && !in_array('blur',$classes) ? ' tooltipster" title="'.esc_html__('Ninguna disponible','pw-calendario').'"' : '')).'"><span class="number">'. $date .'</span></span>';
						$html .= '</td>';
...
function booked_fe_calendar_date_content($date,$calendar_id = false){
	...
	echo '<div class="booked-appt-list">';
		...
		*/
		...
		if ($todays_defaults){
				...
				*/
				...
				if ($spots_available && $available || !$hide_unavailable_timeslots):
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$button_text = (!$spots_available || !$available ? esc_html__('No disponible','pw-calendario') : esc_html( _x('Reservar cita','Book a Single Appointment', 'pw-calendario') ) );
							...
							$html .= '<span class="timeslot-time'.($public_appointments ? ' booked-public-appointments' : '').'">';
								...
								if (!$hide_available_count):
									$html .= '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">';
										$html .= sprintf( _n( '%d plaza libre', '%d plazas libres', $spots_available, 'pw-calendario' ), $spots_available );
									$html .= '</span>';
								endif;

								if ($public_appointments && !empty($appts_in_this_timeslot)):
									$html .= '<span class="booked-public-appointment-title">'._n('Citas en esta franja horaria:','Citas en esta franja horaria:',count($appts_in_this_timeslot),'pw-calendario').'</span>';
			...
			if (!$temp_count):
				...
				echo '<p>'.esc_html__('No hay franjas horarias disponibles este día.','pw-calendario').'</p>';
		...
		*/
		...
		echo '<h2><span>' . sprintf( _n( esc_html(_x( 'Cita disponible el %s', 'Single Appointment', 'pw-calendario' )), esc_html(_x( 'Citas disponibles el %s', 'Multiple Appointments', 'pw-calendario' )), $total_available ), '</span><strong>'.$date_display.'</strong><span>') . '</span></h2>';
		echo $appt_list_html;
	...
	echo '<div class="booked-appt-list" data-list-date="'.$date.'" data-min-date="'.($prevent_before ? date_i18n('Y-m-d',strtotime($prevent_before)) : $earliest_date).'" data-max-date="'.($prevent_after ? date_i18n('Y-m-d',strtotime($prevent_after)) : false).'">';
		...
		*/
		...
		echo '<div class="booked-list-view-nav" data-calendar-id="'.$calendar_id.'">';
			if ($showing_prev):
				echo '<button data-date="'.date_i18n('Y-m-d',strtotime($prev_day)).'" class="booked-list-view-date-prev bb-small"><i class="booked-icon booked-icon-arrow-left"></i>&nbsp;&nbsp;'.esc_html__('Anterior','pw-calendario').'</button>';
			endif;
			if ($showing_next):
				echo '<button data-date="'.date_i18n('Y-m-d',strtotime($next_day)).'" class="booked-list-view-date-next bb-small">'.esc_html__('Siguiente','pw-calendario').'&nbsp;&nbsp;<i class="booked-icon booked-icon-arrow-right"></i></button>';
			endif;
					...
					if ($hide_unavailable_timeslots && !$available):
						...
						$html = '<div class="timeslot bookedClearFix'.($title && $only_titles == true ? ' booked-hide-time' : '').($hide_available_count || !$available ? ' timeslot-count-hidden' : '').(!$available ? ' timeslot-unavailable' : '').($title ? ' has-title ' : '').'">';
							$html .= '<span class="timeslot-time'.($public_appointments ? ' booked-public-appointments' : '').'">';
								...
								$html .= '<span class="timeslot-range"><i class="booked-icon booked-icon-clock"></i>&nbsp;&nbsp;' . $timeslotText . '</span>';
								if (!$hide_available_count): $html .= '<span class="spots-available'.($spots_available == 0 ? ' empty' : '').'">'.sprintf(_n('%d plaza libre','%d plazas libres',$spots_available,'pw-calendario'),$spots_available).'</span>'; endif;
								if ($public_appointments && !empty($appts_in_this_timeslot)):
									$html .= '<span class="booked-public-appointment-title">'._n('Citas en esta franja horaria:','Citas en esta franja horaria:',count($appts_in_this_timeslot),'pw-calendario').'</span>';
									$html .= '<ul class="booked-public-appointment-list">';
									foreach($appts_in_this_timeslot as $appt_id):
		...
		*/
		...
		echo '<h2'.(!$showing_prev ? ' class="booked-no-prev"' : '').'><span>'.sprintf(_n('Cita disponible el %s','Citas disponibles el %s',$total_available,'pw-calendario'),'</span><strong>'.$date_display.'</strong>').'</h2>';
		echo $appt_list_html;
...
function booked_reset_password($user_login){
	...
	/** Compatibilidad: el gancho estandar de WordPress. */
	...
	// Se usa el del usuario para que el correo lleve las mayusculas bien.
	...
	/*
	 * La clave la genera WordPress.
	 *
	 * Antes se fabricaba a mano con PasswordHash y se escribia
	 * directamente en la tabla de usuarios. Eso dejo de funcionar cuando
	 * el nucleo retiro class-phpass.php, pero el problema de fondo era
	 * otro y llevaba mas tiempo: WordPress guarda la clave como
	 * `time():hash`, y check_password_reset_key() necesita ese prefijo
	 * para comprobar la caducidad. Sin el, todos los enlaces que generaba
	 * el plugin se rechazaban como invalidos.
	 *
	 * get_password_reset_key() existe desde WordPress 4.4, se ocupa del
	 * hash y del formato, y respeta el filtro allow_password_reset.
	 */
	...
	$message  = esc_html__('Alguien ha solicitado restablecer la contraseña de esta cuenta:','pw-calendario') . "\r\n\r\n";
	$message .= network_home_url( '/' ) . "\r\n\r\n";
	$message .= sprintf(esc_html__('Usuario: %s','pw-calendario'), $user_login) . "\r\n\r\n";
	$message .= esc_html__('Si ha sido un error, ignora este correo y no pasará nada.','pw-calendario') . "\r\n\r\n";
	$message .= esc_html__('Para restablecer la contraseña, visita esta dirección:','pw-calendario') . "\r\n\r\n";
			...
			switch($field_type):
				...
				case 'drop-down-label' :
					...
					?><div class="field">
						...
						<select id="booked-select-<?php echo esc_attr($field['name']); ?>" <?php echo $data_attributes ?> <?php if ($is_required): echo ' required="required"'; endif; ?> name="<?php echo esc_attr($field['name']); ?>"><option value=""><?php esc_html_e('Elegir…','pw-calendario'); ?></option><?php
...
function booked_profile_update_submit(){
	...
	if (is_user_logged_in()):
		...
		if ( 'POST' === $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['action'] ) && 'update-user' === $_POST['action'] ) {

			// Comprobacion de nonce. Sin ella, otro sitio podia provocar que
			// un usuario identificado cambiase su correo sin darse cuenta y
			// tomarle la cuenta con un restablecimiento de contrasena.
			if ( ! isset( $_POST['pwcal_perfil_nonce'] )
				|| ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['pwcal_perfil_nonce'] ) ), 'pwcal_actualizar_perfil' ) ) {
				$error[] = esc_html__( 'La comprobación de seguridad ha fallado. Recarga la página e inténtalo de nuevo.', 'pw-calendario' );
				return;
			}
			...
			if ( ! empty( $_POST['pass1'] ) && ! empty( $_POST['pass2'] ) ) {
				...
				// Las contrasenas NO se sanean: `sanitize_text_field` altera
				// los caracteres especiales y corromperia la contrasena.
				$pwcal_pass1 = (string) wp_unslash( $_POST['pass1'] );
				$pwcal_pass2 = (string) wp_unslash( $_POST['pass2'] );

				if ( $pwcal_pass1 !== $pwcal_pass2 ) {

					$error[] = esc_html__( 'Las contraseñas no coinciden. No se ha cambiado la contraseña.', 'pw-calendario' );

				} elseif ( strlen( $pwcal_pass1 ) < 8 ) {

					$error[] = esc_html__( 'La contraseña debe tener al menos 8 caracteres.', 'pw-calendario' );

				} else {

					wp_update_user(
						array(
							'ID'        => $booked_current_user->ID,
							'user_pass' => $pwcal_pass1,
						)
					);

					// Cerrar las demas sesiones tras cambiar la contrasena.
		    ...
		    if ( isset( $_POST['url'] ) )
		    	wp_update_user( array( 'ID' => $booked_current_user->ID, 'user_url' => esc_url( pwcal_post_texto( 'url' ) ) ) );
		    if ( isset( $_POST['email'] ) ){

		    	$email_exists = email_exists(pwcal_post_texto( 'email' ));

		        if (!is_email(pwcal_post_texto( 'email' )))
		            $error[] = esc_html__('El correo electrónico que has escrito no es válido. Inténtalo de nuevo.', 'pw-calendario');
		        elseif( $email_exists && $email_exists != $booked_current_user->ID )
		            $error[] = esc_html__('Ese correo electrónico ya lo usa otro usuario. Prueba con otro.', 'pw-calendario');
		        else{
		            wp_update_user( array ('ID' => $booked_current_user->ID, 'user_email' => pwcal_post_texto( 'email' )));
		        }
		    }

		    if ( isset( $_POST['nickname'] ) ):
		        update_user_meta( $booked_current_user->ID, 'nickname', pwcal_post_texto( 'nickname' ) );
		        wp_update_user( array ('ID' => $booked_current_user->ID, 'display_name' => pwcal_post_texto( 'nickname' )));
		    endif;
	        ...
	        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] == 0):
			    ...
			    if (isset($avatar,$_POST['avatar_nonce']) && $avatar && wp_verify_nonce( $_POST['avatar_nonce'], 'avatar_upload' )) {
				    ...
				    if ( is_wp_error( $attachment_id ) ) {
				        $error[] = esc_html__('Se ha producido un error al subir el avatar.','pw-calendario');
				    } else {
			    	...
			    	$error[] = esc_html__('La comprobación de seguridad de la subida del avatar ha fallado.','pw-calendario');
...
function booked_wpml_ajax(){
	if ( isset( $_GET['wpml_lang'] ) && $_GET['wpml_lang'] ):
    	do_action( 'wpml_switch_language', sanitize_key( wp_unslash( $_GET['wpml_lang'] ) ) );
	endif;
}
...
function booked_profile_content_appointments(){
	...
	if ( apply_filters( 'booked_sessions_enabled', true ) ){
		if (isset($_SESSION['appt_requested']) && isset($_SESSION['new_account'])){
			...
			echo '<p class="booked-form-notice">'.esc_html__('Hemos recibido tu solicitud de cita y te hemos creado una cuenta. Te enviamos los datos de acceso por correo electrónico. Cuando accedas verás aquí abajo tus citas próximas. Te recomendamos cambiar la contraseña por una que recuerdes mejor desde la pestaña \\"Editar el perfil\\".','pw-calendario').'</p>';
			...
			if ($appointment_default_status == 'draft'):
				echo '<p class="booked-form-notice">'.esc_html__('Hemos recibido tu solicitud de cita. Si se aprueba, la verás actualizada aquí abajo.','pw-calendario').'</p>';
			else :
				echo '<p class="booked-form-notice">'.esc_html__('Tu cita ya está en nuestro calendario.','pw-calendario').'</p>';
			endif;
...
function booked_profile_content_edit(){
	...
	echo '<h4><i class="booked-icon booked-icon-edit" style="position:relative; top:-1px;"></i>&nbsp;&nbsp;'.esc_html__('Editar el perfil','pw-calendario').'</h4>'; ?>

    <form method="post" enctype="multipart/form-data" id="booked-page-form" action="<?php the_permalink(); ?>">

		<?php wp_nonce_field( 'pwcal_actualizar_perfil', 'pwcal_perfil_nonce' ); ?>

	    <div class="bookedClearFix">
            <p class="form-avatar">
                <label for="avatar"><?php esc_html_e('Cambiar el avatar', 'pw-calendario'); ?></label><br>
                <span class="booked-upload-wrap"><span><?php esc_html_e('Elegir imagen…','pw-calendario'); ?></span><input class="field" name="avatar" type="file" id="avatar" value="" /></span>
                <?php wp_nonce_field( 'avatar_upload', 'avatar_nonce' ); ?>
                <span class="hint-p"><?php esc_html_e('Tamaño recomendado: 100 x 100 px o mayor', 'pw-calendario'); ?></span>
            </p><!-- .form-nickname -->
    	</div>

        <div class="bookedClearFix">
            <p class="form-nickname">
                <label for="nickname"><?php esc_html_e('Nombre visible', 'pw-calendario'); ?></label>
                <input class="text-input" name="nickname" type="text" id="nickname" value="<?php echo esc_attr( get_the_author_meta( 'nickname', $booked_current_user->ID ) ); ?>" />
            </p><!-- .form-nickname -->
            <p class="form-email">
                <label for="email"><?php esc_html_e('Correo electrónico *', 'pw-calendario'); ?></label>
                <input class="text-input" name="email" type="text" id="email" value="<?php echo esc_attr( get_the_author_meta( 'user_email', $booked_current_user->ID ) ); ?>" />
            </p><!-- .form-email -->
        </div>
        <div class="bookedClearFix">
            <p class="form-password">
                <label for="pass1"><?php esc_html_e('Cambiar la contraseña', 'pw-calendario'); ?></label>
                <input class="text-input" name="pass1" type="password" id="pass1" />
            </p><!-- .form-password -->
            <p class="form-password last">
                <label for="pass2"><?php esc_html_e('Repite la contraseña', 'pw-calendario'); ?></label>
        ...
        ?>
        <p class="form-submit">
            <input name="updateuser" type="submit" id="updateuser" class="submit button button-primary" value="<?php esc_html_e('Actualizar', 'pw-calendario'); ?>" />
