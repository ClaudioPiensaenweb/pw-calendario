...
class Booked_WC_Product {
	...
	public static function get( $post_id = null ) {
		...
		if ( !is_integer( $post_id ) ) {
			$message = sprintf( __('Se esperaba un entero (%s) y se ha recibido %s.', 'pw-calendario'), 'Booked_WC_Product::get($post_id)', gettype($post_id) );
	...
	protected function _get_product_data() {
		...
		if ( !$this->data ) {
			//$message = sprintf(__('Se ha producido un error al recuperar los datos del producto con ID %1$d.', 'pw-calendario'), $this->post_id);
			//throw new Exception($message);
		}
