...
<?php if ($post_status != 'publish' && $post_status != 'future'): ?>
	<button data-appt-id="<?php echo $appt_id ?>" class="approve button button-primary"><?php echo __('Aprobar la cita', 'pw-calendario') ?></button>
<?php endif; ?>
	...
	<span class="booked-wc_status-text awaiting">
		...
		if ( $appointment->order_id ) {
			...
			if (current_user_can('manage_booked_options')) :
				echo '<button data-appt-id="'.$appt_id.'" class="mark-paid button">'.__('Marcar como pagada', 'pw-calendario').'</button>';
			...
			echo '<span>' . __('Pendiente de pago', 'pw-calendario') . '</span>';
...
<?php else:
	...
	echo '<span class="booked-wc_status-text paid">';
		...
		if (current_user_can('manage_booked_options') && $appointment->order_id != 'manual') :
			echo '<a target="_blank" href="' . admin_url('/post.php?post=' . $appointment->order_id . '&action=edit') . '"><i class="booked-icon booked-icon-pencil"></i>&nbsp;&nbsp;' . __('Pagada', 'pw-calendario') . '</a>';
		else :
			echo '<span>' . __('Pagada', 'pw-calendario') . '</span>';
		endif;
