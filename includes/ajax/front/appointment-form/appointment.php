...
$all_day_text = esc_html__( 'Todo el día', 'pw-calendario' );
...
?><div class="booked-appointment-details" data-appt-key="<?php echo $appointment_key; ?>"><?php
	...
	if ( $appt_timeslot == $all_day_text ):
		...
		if ( get_option('booked_show_only_titles') && $appt_title ):
			...
			?><p class="appointment-info"><i class="booked-icon booked-icon-calendar"></i>&nbsp;&nbsp;&nbsp;<?php echo sprintf( esc_html__( '%s a las %s','pw-calendario' ), $appt_date_name, $appt_timeslot ); ?></p><?php
		endif;
	endif;
