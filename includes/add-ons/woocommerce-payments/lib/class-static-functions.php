...
class Booked_WC_Functions {
	...
	public static function get_calendar_page_info( $calendar_id = null ) {
		...
		if ( $calendar_id ) {
			...
			// Se buscan las tres formas de escribir el atributo en el
			// shortcode: sin comillas, con dobles y con simples.
			$pages = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT `ID`, `post_title`, `post_name`, `post_type` FROM `{$wpdb->posts}`
					 ...
					 LIMIT 1",
					'%' . $wpdb->esc_like( 'calendar=' . $calendar_id ) . '%',
					'%' . $wpdb->esc_like( 'calendar="' . $calendar_id . '"' ) . '%',
					'%' . $wpdb->esc_like( "calendar='" . $calendar_id . "'" ) . '%'
				)
			);
		...
		if ( !$pages ) {
			...
			// El original comparaba con el shortcode exacto y sin comodines,
			// asi que nunca encontraba nada.
			$pages = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT `ID`, `post_title`, `post_name`, `post_type` FROM `{$wpdb->posts}`
					 WHERE `post_type` = 'page'
		...
		$pages = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT `ID`, `post_title`, `post_name`, `post_type` FROM `{$wpdb->posts}`
				 ...
				 AND `post_content` LIKE %s
				 LIMIT 1",
				'%' . $wpdb->esc_like( '[booked-appointments]' ) . '%'
			)
		);
		if ( !$pages ) {
			return;
	...
	public static function booked_new_appointment_form() {
		...
		if (
			!empty($_POST['action'])
			&& pwcal_post_texto( 'action' )==='booked_new_appointment_form'
			&& !empty($_POST['source'])
			&& pwcal_post_texto( 'source' )==='booked_wc_extension'
		) {
			...
			if ( $calendar_id ) {
				...
				if ( $assignee_email && ($usr=get_user_by('email', $assignee_email)) ) {
					$custom_field_data['booking-agent'] = array(
						'label' => esc_attr__('Gestor de citas', 'pw-calendario'),
	...
	public static function booked_fea_shortcode_appointments_buttons($button_html, $appt_id) {
		...
		if ($appointment->order_id):
			...
			$button_html = '<div class="booked-fea-buttons">';
				$button_html .= ($status_class == 'pending' ? '<button data-appt-id="'.$appt_id.'" class="approve button button-primary">'.__('Aprobar','pw-calendario').'</button>' : '');
			$button_html .=	'</div>';
			return $button_html;
		else:
			return $button_html;
	...
	public static function booked_button_book_appointment( $label ) {
		...
		if (
			...
			|| pwcal_post_texto( 'app_action' )!=='edit'
			|| empty($_POST['source'])
			|| pwcal_post_texto( 'source' )!=='booked_wc_extension'
		) {
			return $label;
		...
		$appointment = Booked_WC_Appointment::get(pwcal_post_entero( 'app_id' ));
		$current_time = current_time('timestamp');
		...
		return '<span class="button-text" >' . __('Elegir una fecha nueva', 'pw-calendario') . '</span></button></span>';
	}
	...
	public static function booked_before_creating_appointment() {
		...
		if ($customer_type == 'guest'):
			$first_name = pwcal_post_texto( 'guest_name' );
			$last_name = isset($_POST['guest_surname']) && $_POST['guest_surname'] ? pwcal_post_texto( 'guest_surname' ) : false;
			$email = isset($_POST['guest_email']) ? pwcal_post_texto( 'guest_email' ) : '';
		elseif ($customer_type == 'new'):
			$first_name = pwcal_post_texto( 'booked_appt_name' );
			$last_name = ( isset($_POST['booked_appt_surname']) && $_POST['booked_appt_surname'] ? pwcal_post_texto( 'booked_appt_surname' ) : false );
	...
	public static function booked_new_appointment_args_on_date_change( $post_args ) {
		...
		$appointment_id = pwcal_post_entero( 'app_id' );
		...
		$post_args['ID'] = pwcal_post_entero( 'app_id' );
