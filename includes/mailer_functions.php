...
function booked_mailer_tokens(){
	return apply_filters( 'booked_mailer_tokens', array(
		'name' => esc_html__( "Muestra el nombre completo del cliente.",'pw-calendario' ),
		'email' => esc_html__( "Muestra el correo electrónico del cliente.",'pw-calendario' ),
		'title' => esc_html__( "Muestra el título de la franja horaria de la cita.",'pw-calendario' ),
		'calendar' => esc_html__( "Muestra el nombre del calendario de la cita, si tiene uno.",'pw-calendario' ),
...
function booked_user_tokens(){
	return apply_filters( 'booked_user_tokens', array(
		'name' => esc_html__( "Muestra el nombre del cliente.",'pw-calendario' ),
		'username' => esc_html__( "Muestra el usuario del cliente.",'pw-calendario' ),
		'password' => esc_html__( "Muestra la contraseña del cliente.",'pw-calendario' ),
		'email' => esc_html__( "Muestra el correo electrónico del cliente.",'pw-calendario' )
	));
}
...
function booked_get_appointment_tokens( $appt_id ){
	...
	if ($timeslots[0] == '0000' && $timeslots[1] == '2400'):
		$time_text = esc_html__( 'Todo el día', 'pw-calendario' );
	else :
...
function booked_mailer( $to = false, $subject = '', $message = '', $from_email = false, $from_name = false ){
	...
	elseif ( file_exists( get_template_directory() . '/booked/email-template.html' ) ):
		...
		$template = file_get_contents( untrailingslashit( PWCAL_PLUGIN_DIR ) . '/includes/email-templates/default.html', true );
	endif;
