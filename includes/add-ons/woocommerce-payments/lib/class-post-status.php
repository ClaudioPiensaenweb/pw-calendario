...
class Booked_WC_Post_Status {
	...
	protected function register_status() {
		register_post_status(
			...
			array(
				'label'                     => _x('Pendiente de pago', 'pw-calendario'),
				'public'                    => true,
	...
	public function append_post_status_list() {
		...
		if( $post && $post->post_type===BOOKED_WC_POST_TYPE ) {
			...
			if( $post->post_status===$post_status ) {
				...
				$label = '<span id=\"post-status-display\"> ' . __('Pendiente de pago', 'pw-calendario') . '</span>';
			}

			echo '
				<script type="text/javascript">
					jQuery(document).ready(function($){
						$("select#post_status").append("<option value=\"' . $post_status . '\" ' . $complete . '>' .  __('Pendiente de pago', 'pw-calendario') . '</option>");
	...
	public function display_post_states( $states ) {
		...
		if($arg!==$post_status && $post->post_status===$post_status) {
			return array(__('Pendiente de pago', 'pw-calendario'));
		}
