...
function booked_get_profile_page(){
	...
	// Consulta preparada: `esc_like` escapa los comodines y `prepare`
	// se encarga del entrecomillado.
...
function booked_registration_validation( $email, $password, $captcha_value = false, $captcha_from_user = false )  {
	...
	if ($captcha_value):
		if ($captcha_value != $captcha_from_user):
			$reg_errors->add('captcha', esc_html__('El texto que has escrito no coincide con el de la imagen.','pw-calendario'));
		else :
	...
	if ( !$email || !$password ) {
	    $reg_errors->add('field', esc_html__('Todos los campos son obligatorios para registrarte.','pw-calendario'));
	}

	if ( username_exists( $email ) ) {
    	$reg_errors->add('user_name', esc_html__('Ese nombre de usuario ya existe.','pw-calendario'));
    }

    if ( ! validate_username( $email ) ) {
	    $reg_errors->add( 'username_invalid', esc_html__('Ese nombre no es válido.','pw-calendario'));
	}

    if ( !is_email( $email ) ) {
	    $reg_errors->add( 'email_invalid', esc_html__('Ese correo electrónico no es válido.','pw-calendario'));
	}

	if ( email_exists( $email ) ) {
	    $reg_errors->add( 'email', esc_html__('Ese correo electrónico ya está en uso.','pw-calendario'));
	}
...
function booked_complete_registration() {
    ...
    if ( 1 > count( $reg_errors->get_error_messages() ) ) {
        ...
        return '<p class="booked-form-notice"><strong>'.esc_html__('¡Hecho!','pw-calendario').'</strong><br />'.esc_html__('Registro completado. Revisa tu correo electrónico: te hemos enviado los datos de acceso.','pw-calendario').'</p>';
...
function booked_registration_form($name, $surname, $email, $password){
	...
	?><form action="" method="post" class="wp-user-form">
		...
		<p class="name">
			<label for="name"><?php echo ( $name_requirements == 'require_surname' ? esc_html__('Nombre','pw-calendario') : esc_html__('Tu nombre','pw-calendario') ); ?></label>
			<input type="text" name="booked_reg_name" value="<?php echo isset( $name ) ? esc_attr( $name ) : ''; ?>" id="name" tabindex="101" />
		</p>
		<?php if ($name_requirements == 'require_surname'): ?>
		<p class="surname">
			<label for="surname"><?php esc_html_e('Apellidos','pw-calendario'); ?></label>
			<input type="text" name="booked_reg_surname" value="<?php echo isset( $surname ) ? esc_attr( $surname ) : ''; ?>" id="surname" tabindex="102" />
		</p>
		<?php endif; ?>
		<p class="email">
			<label for="email"><?php esc_html_e('Correo electrónico','pw-calendario'); ?></label>
			<input type="email" name="booked_reg_email" value="<?php echo isset( $email ) ? esc_attr( $email ) : ''; ?>" id="email" tabindex="103" />
		</p>
		<p class="password">
			<label for="password"><?php esc_html_e('Elige una contraseña','pw-calendario'); ?></label>
		...
		<?php if (class_exists('ReallySimpleCaptcha')) :
			...
			?><p class="captcha">
				<label for="captcha_code"><?php esc_html_e('Escribe el siguiente texto:','pw-calendario'); ?></label><?php
		...
		<input type="submit" name="booked_reg_submit" value="<?php esc_html_e('Registrarse','pw-calendario'); ?>" class="user-submit button-primary" tabindex="105" />
