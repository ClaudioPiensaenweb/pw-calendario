...
class Booked_WC_Order {
	...
	public static function get( $order_id=null ) {
		if ( !is_integer($order_id) ) {
			$message = sprintf( __('Se esperaba un entero (%s) y se ha recibido %s.', 'pw-calendario'), 'Booked_WC_Order::get($order_id)', gettype($order_id) );
		...
		foreach ($cart_appointments['ids'] as $app_id) {
			...
			foreach ($appointment->products as $product) {
				...
				if ( !in_array($check_key, $cart_appointments['extended']) ) {
					$message = sprintf( __('La cita «%s» y los productos del carrito no coinciden. Comprueba que todos los productos de la cita están en el carrito.', 'pw-calendario'), $appointment->timeslot_text );
					throw new Exception( $message );
				}
			}
		}
