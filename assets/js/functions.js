/**
 * Pw Calendario: nonce en todas las peticiones AJAX.
 *
 * El servidor rechaza con un 403 cualquier accion de escritura que llegue
 * sin nonce valido. Este prefiltro lo adjunta automaticamente a las
 * peticiones dirigidas a admin-ajax.php, tanto si `data` es un objeto como
 * si es una cadena ya serializada.
 */
;(function($) {
	...
	function pwcalNonce() {
		if ( typeof booked_js_vars !== 'undefined' && booked_js_vars.nonce ) {
			return booked_js_vars.nonce;
		}
		return '';
	}
	...
	$.ajaxPrefilter(function(options) {
		...
		var nonce = pwcalNonce();

		if ( ! nonce ) {
			return;
		}

		// Solo se anade a las peticiones al gestor de AJAX de WordPress.
		...
		if ( typeof options.data === 'string' ) {
			if ( options.data.indexOf('nonce=') === -1 ) {
				options.data += ( options.data.length ? '&' : '' ) + 'nonce=' + encodeURIComponent(nonce);
			}
...
;(function($) {
	...
	function cerrarTodos($excepto) {
		$('.pwcal-atc-menu').not($excepto).attr('hidden', true);
		$('.pwcal-atc-boton').not($excepto ? $excepto.siblings('.pwcal-atc-boton') : null)
			.attr('aria-expanded', 'false');
	}

	$(document).on('click', '.pwcal-atc-boton', function(e) {

		e.preventDefault();
		e.stopPropagation();

		var $boton = $(this),
			$menu  = $boton.siblings('.pwcal-atc-menu'),
			abierto = $menu.attr('hidden') === undefined;
		...
		// Si el menu se sale por la derecha de la ventana, se alinea al
		// otro lado del boton.
		var $envoltorio = $boton.parent('.pwcal-atc');
		$envoltorio.removeClass('pwcal-atc-derecha');

		var rect = $menu[0].getBoundingClientRect();

		if ( rect.right > $(window).width() ) {
			$envoltorio.addClass('pwcal-atc-derecha');
		}
	});
	...
	$(document).on('keydown', function(e) {
		if ( e.key === 'Escape' || e.keyCode === 27 ) {
			$('.pwcal-atc-menu:not([hidden])').each(function() {
				$(this).attr('hidden', true)
					.siblings('.pwcal-atc-boton').attr('aria-expanded', 'false').focus();
			});
		}
	});

	// Al elegir una opcion, el menu se cierra.
	$(document).on('click', '.pwcal-atc-menu a', function() {
		$(this).closest('.pwcal-atc-menu').attr('hidden', true)
			.siblings('.pwcal-atc-boton').attr('aria-expanded', 'false');
	});
